...
helper.authenticate = async (req, res, next) => {
  try {
    ...
    if (data) {
      // Check WorkersModel first (for staff/workers)
      let user = await WorkersModel.findOne({ _id: data._id }).lean();

      // If not found, check UsersModel (for admin/manager/supervisor)
      if (!user) {
        user = await UsersModel.findOne({ _id: data._id }).lean();
      }
