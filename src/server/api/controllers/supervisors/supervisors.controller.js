...
controller.teamList = async (req, res) => {
  try {
    const { user, query } = req;
    const data = await service.teamList(user, query);
    return res
      .status(200)
      .json({ statusCode: 200, message: "success", ...data });
  } catch (error) {
...
controller.teamHistory = async (req, res) => {
  try {
    const { user, query, params } = req;
    const data = await service.teamHistory(user, query, params.id);
    return res
      .status(200)
      .json({ statusCode: 200, message: "success", ...data });
  } catch (error) {
