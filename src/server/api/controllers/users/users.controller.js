...
controller.setPermissions = async (req, res) => {
  try {
    ...
    if (user.role !== "admin") {
      return res
        .status(403)
        .json({ message: "Forbidden: Only admin can set permissions." });
    }
    const { userId, permissions } = body;
    if (!userId || !permissions) {
      return res
        .status(400)
        .json({ message: "userId and permissions are required." });
    }
    const data = await service.setPermissions(userId, permissions);
    return res
      .status(200)
      .json({ statusCode: 200, message: "Permissions updated", data });
  } catch (error) {
    console.error(error);
